    ...
    if(isset($_POST["btn_submit_register"]) && !empty($_POST["btn_submit_register"])){
        ...
        if(isset($_POST["captcha"]) && !empty($captcha)){
            ...
            if(($_SESSION["rand"] != $captcha) && ($_SESSION["rand"] != "")){
                ...
                $error_message = "<p class='mesage_error text-secondary'><strong>Ошибка!&nbsp;</strong>Неправильная капча</p>";
            ...
            if(!$result_query_insert){
                ...
                if(!$query_insert_confirm){
                    ...
                    $message = 'Здравствуйте! <br/> <br/> Сегодня '.date("d.m.Y", time()).', неким пользователем была произведена регистрация на сайте <a href="'.$_SERVER['HTTP_HOST'].'">'.$_SERVER['HTTP_HOST'].'</a> используя Ваш email. Если это были Вы, то, пожалуйста, подтвердите адрес вашей электронной почты, перейдя по этой ссылке: <a href="'.$_SERVER['HTTP_HOST'].'/app/auth/activation.php?token='.$token.'&email='.$email.'">АКТИВАЦИЯ АККАУНТА.</a> <br/> <br/> В противном случае, если это были не Вы, то, просто игнорируйте это письмо. <br/> <br/> <strong>Внимание!</strong> Ссылка действительна 24 часа. После чего Ваш аккаунт будет удален из базы.';
                    ...
                    if(mail($email, $subject, $message, $headers)){
                        $_SESSION["success_messages"] = "<h4 class='success_message text-success'><strong>Регистрация прошла успешно!!!</strong></h4><p class='success_message'> Теперь необходимо подтвердить введенный адрес электронной почты. Для этого, перейдите по ссылке указанной в сообщении, которую получили на почту ".$email." </p>";
